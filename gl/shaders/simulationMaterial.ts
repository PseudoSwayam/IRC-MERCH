  ...
  constructor(scale: number = 10.0) {
    ...
    super({
      ...
      void main() {
        ...
        // Smooth continuous time for wave motion
        float continuousTime = uTime * uTimeScale * (6.28318530718 / uLoopPeriod);
        
        // Create smooth wave patterns based on position
        float waveX = sin(originalPos.z * 2.0 + continuousTime) * 0.5;
        float waveZ = sin(originalPos.x * 2.0 + continuousTime * 0.8) * 0.5;
        
        // Smooth vertical wave with radial influence
        float distanceFromCenter = length(originalPos.xz);
        float radialWave = sin(distanceFromCenter * 1.5 - continuousTime * 1.5) * 0.8;
        
        // Combine waves for smooth, coordinated movement
        float displacementX = waveX * uNoiseIntensity;
        float displacementY = radialWave * uNoiseIntensity * 1.2; // Primary vertical wave
        float displacementZ = waveZ * uNoiseIntensity;
        
        // Add subtle secondary wave for depth
        float secondaryWave = sin(originalPos.x * 1.2 + originalPos.z * 1.2 + continuousTime * 0.6) * 0.3;
        displacementY += secondaryWave * uNoiseIntensity;
